...
const RESULTS_DIR = 'model/flux-1-kontext-dev/results';
const PROMPTS_FILE = 'example_inputs/edit-prompts.txt';
const IMAGES_DIR = 'example_inputs/edit-images';
...
async function saveImageData(imageData, filename) {
  try {
    ...
    // If imageData is an object with url property
      ...
      const buffer = await response.arrayBuffer();
      // Always save with original filename (PNG extension) regardless of content type
      await fs.writeFile(path.join(RESULTS_DIR, filename), Buffer.from(buffer));
      return { 
        success: true, 
        type: 'object_url', 
      ...
      try {
        ...
        if (result.images && result.images.length > 0) {
          for (let j = 0; j < result.images.length; j++) {
            const filename = `edit_${editNumber.toString().padStart(2, '0')}_${path.basename(imageName, path.extname(imageName))}_result_${j + 1}.png`;
