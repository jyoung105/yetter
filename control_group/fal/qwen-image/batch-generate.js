...
const RESULTS_DIR = 'control_group/fal/qwen-image/results';
...
async function processPrompts() {
  console.log("🎨 Starting Fal AI Qwen-Image batch image generation from prompts.txt");
  ...
  try {
    ...
    for (let i = 0; i < prompts.length; i++) {
      ...
      try {
        ...
        // Generate image with Fal AI Qwen-Image (optimized settings)
    ...
    // Save summary report
    const reportText = `# Fal AI Qwen-Image Batch Image Generation Report
...
${results.map(r => {
  if (r.success) {
    ...
    return `✅ Prompt ${r.promptNumber}: "${r.prompt.substring(0, 50)}..." - ${r.imagesGenerated} images (${r.executionTime.toFixed(2)}s)${nsfwStr}`;
  } else {
...
`;
    ...
    console.log("\n🎉 Fal AI Qwen-Image batch processing completed!");
    ...
    if (summary.nsfwDetections > 0) {
      console.log(`⚠️ NSFW content detected in ${summary.nsfwDetections} generation(s)`);
    }
    ...
    console.error("❌ Fal AI Qwen-Image batch processing failed:", error.message);
    throw error;
  }
}
