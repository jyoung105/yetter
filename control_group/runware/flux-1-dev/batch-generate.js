...
const RESULTS_DIR = 'control_group/runware/flux-1-dev/results';
...
async function processPrompts() {
  console.log("🎨 Starting Runware Flux-Dev batch image generation from prompts.txt");
  ...
  try {
    ...
    for (let i = 0; i < prompts.length; i++) {
      ...
      try {
        ...
        // Generate image with Runware Flux-Dev (optimized settings)
    ...
    // Save summary report
    const reportText = `# Runware Flux-Dev Batch Image Generation Report
...
`;
    ...
    console.log("\n🎉 Runware Flux-Dev batch processing completed!");
    ...
    console.error("❌ Runware Flux-Dev batch processing failed:", error.message);
    throw error;
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  processPrompts()
    .then(() => {
      console.log("✅ Flux-Dev batch processing completed successfully");
      process.exit(0);
    })
    .catch(error => {
